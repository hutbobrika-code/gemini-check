...
HELP = (
    "<b>Бот проверки доступности</b>\n\n"
    "/status — проверить Gemini, ChatGPT, Claude, YouTube и другие площадки "
    "через все прокси и узлы подписки, со сроками аренды и файлом со списком\n"
    "/check <i>адрес</i> — проверить любой сайт через те же точки, "
    "например <code>/check youtube.com</code>\n"
    "/replaced — какие адреса меняли, когда и из-за чего\n\n"
    "Сам по себе бот молчит: сообщение приходит, когда точка упала или "
    "поднялась, и когда продавец выдал новый адрес взамен битого."
)
